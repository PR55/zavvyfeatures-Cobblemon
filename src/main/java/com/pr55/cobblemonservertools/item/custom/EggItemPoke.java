...
public class EggItemPoke extends Item {
    ...
    String ACTIVATE_GUIDE = "tooltip.cobblemonservertools.common";
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if(stack.get(ModDataComponentTypes.TIMER) != null){
            ...
            if(stack.get(ModDataComponentTypes.TIMER) <= 0){
                tooltip.add(Text.translatable("tooltip.cobblemonservertools.egg_item_done.tooltip"));
            }else{
                tooltip.add(Text.translatable("tooltip.cobblemonservertools.egg_item.tooltip"));
...
// /give Player309 cobblemonservertools:egg_item_rare[cobblemonservertools:active=true,cobblemonservertools:seconds=0,cobblemonservertools:hatchable=true,cobblemonservertools:timer=0]
    public void handleRandomUse(ItemStack stack, PlayerEntity user){
        ...
        if(isAlpha()){
            token.set(ModDataComponentTypes.ALPHA, true);
        }else {
            token.set(ModDataComponentTypes.ALPHA, false);
        }
        // Alolan Bias Check
